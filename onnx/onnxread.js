class DdddOcr {
    constructor() {
        this.use_import_onnx = false;
        this.__word = false;
        this.__resize = [];
        this.__channel = 1;
        this.__graph_path = chrome.runtime.getURL('onnx/common.onnx');
        this.__charset = ["", "笤", "谴", "膀", "荔", "佰", "电", "臁", "矍", "同", "奇", "芄", "吠", "6",
        ...
        "友", "唉", "怫", "荘"];
        this.__ort_session = null;
        this.__providers = [
            'CPUExecutionProvider'
        ];
    

        this.__ort_session = new onnx.InferenceSession({
        model:  this.__graph_path ,
    ...
    async classification(img) {
        ...
        if (!(img instanceof Uint8Array)) {
            throw new TypeError('Image should be of type Uint8Array.');
        }
        const image = await createImageBitmap(new Blob([img]));
    /*
        ...
        const resizedImage = image.resize([Math.round(image.width * (64 / image.height)), 64], Image.ANTIALIAS);
        const inputTensor = ndarray(new Float32Array(resizedImage.data.map(x => (x / 255.0 - 0.5) / 0.5)), [1, 1, resizedImage.height, resizedImage.width]);
        const ortInputs = {
            input1: inputTensor.data
        };
        */
        const canvas = document.createElement('canvas');
        const ctx = canvas.getContext('2d');
        ...
        const grayData = new Uint8ClampedArray(newWidth * newHeight);
        for (let i = 0; i < imageData.data.length; i += 4) {
        const r = imageData.data[i];
        const g = imageData.data[i + 1];
        const b = imageData.data[i + 2];
        grayData[i / 4] = 0.2989 * r + 0.5870 * g + 0.1140 * b;
        }
        // 将数据转换为 Float32Array
        const inputTensor = new Float32Array(grayData).map(x => (x / 255.0 - 0.5) / 0.5);
        // 将数据重新形状为 4D 张量，用于输入到 ONNX.js 模型中
        let reshapedTensor;
        await chrome.runtime.sendMessage( { data: inputTensor,height:newHeight,width:newWidth }, function(response) {
            console.log(response);
            console.log("1");
            reshapedTensor=response;
        });
         // 创建模型输入
         console.log("2");
         const ortInputs = {
            input1: reshapedTensor.data
            };
            // 运行模型
            const ortOuts = await this.__ort_session.run(ortInputs);
            const result = [];
            let lastItem = 0;
            if (this.__word) {
                for (const item of ortOuts[1]) {
                result.push(this.__charset[item]);
                }
            } else {
                for (const item of ortOuts[0][0]) {
                if (item === lastItem) {
                    continue;
                } else {
                    lastItem = item;
